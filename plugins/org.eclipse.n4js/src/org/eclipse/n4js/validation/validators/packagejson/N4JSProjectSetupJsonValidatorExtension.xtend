...
public class N4JSProjectSetupJsonValidatorExtension extends AbstractPackageJSONValidatorExtension {
	 ...
	 */
	private def holdsProjectWithTestFragmentDependsOnTestLibrary(N4JSProjectConfigSnapshot project) {
	 ...
	 */
	private def boolean anyDependsOnTestLibrary(List<? extends N4JSProjectConfigSnapshot> projects) {
	...
	static class HasTestDependencyVisitor implements DependencyVisitor<N4JSProjectConfigSnapshot> {
		...
		private def boolean hasTestDependency(N4JSProjectConfigSnapshot p) {
			for (N4JSProjectConfigSnapshot pDep : p.dependenciesOLD) {
				if ((N4JSGlobals.VENDOR_ID.equals(pDep.vendorId) && (N4JSGlobals.MANGELHAFT.equals(pDep.name))
					|| N4JSGlobals.MANGELHAFT_ASSERT.equals(pDep.name)
				)) {
					return true;
				}
			}
	...
	def checkExternalProjectDoesNotReferenceWorkspaceProject(JSONValue dependenciesValue) {
		...
		stack.addAll(currentProject.allDirectDependencies.filter(N4JSProjectConfigSnapshot).filter[external]);
		...
		while (!stack.isEmpty) {
			...
			val actualDirectDependencies = actual.allDirectDependencies.filter(N4JSProjectConfigSnapshot);
	...
	private def void checkImplProjectPresentForReferencedTypeDef(ValidationProjectReference ref,
		N4JSProjectConfigSnapshot referencedProject, Set<N4JSProjectName> allReferencedProjectNames) {

		if (referencedProject.type === ProjectType.DEFINITION) {
			val nameOfProjectDefinedByReferencedProject = referencedProject.definesPackage;
	...
	private def checkVersions(N4JSProjectConfigSnapshot curPrj, ValidationProjectReference ref, N4JSProjectName id, Map<N4JSProjectName, N4JSProjectConfigSnapshot> allProjects) {
	 ...
	 */
	private def Map<N4JSProjectName, N4JSProjectConfigSnapshot> getAllProjectsByName() {
