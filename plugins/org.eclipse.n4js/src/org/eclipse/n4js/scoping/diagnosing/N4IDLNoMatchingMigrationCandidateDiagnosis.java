...
public class N4IDLNoMatchingMigrationCandidateDiagnosis extends ScopingDiagnosis<IdentifierRef> {
	...
	DiagnosticMessage diagnose(QualifiedName name, IdentifierRef identifierRef) {
		...
		final List<TMigration> candidates = migrationLocator.getMigrationCandidates(migrateCall.getArguments());
		...
		final String argumentsDescription = MigrationUtils.getMigrationArgumentsDescription(argumentTypeRefs);
