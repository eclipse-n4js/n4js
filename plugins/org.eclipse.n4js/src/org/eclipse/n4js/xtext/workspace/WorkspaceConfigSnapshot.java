...
public class WorkspaceConfigSnapshot {
	...
	public ProjectConfigSnapshot findProjectByNestedLocation(URI nestedLocation) {
		return findInMapByNestedURI(projectPath2Project, nestedLocation);
	}
	...
	public ProjectConfigSnapshot findProjectContaining(URI nestedLocation) {
		ProjectConfigSnapshot candidate = findInMapByNestedURI(sourceFolderPath2Project, nestedLocation);
		if (candidate != null) {
	...
	/**
	 * Returns the value for the key that is the longest prefix of the given URI or <code>null</code> if none of the
	 * given map's keys is a prefix of the given URI. Assumes keys to be URIs <em>without</em> a trailing path
	 * separator.
	 */
	protected <T> T findInMapByNestedURI(Map<URI, T> map, URI nestedLocation) {
		URI currPrefix = trimTrailingPathSeparator(nestedLocation);
		do {
	...
	protected void updateLookupMaps(
		...
		for (ProjectConfigSnapshot project : changedProjects) {
			...
			if (oldProject != null) {
				lookupProjectPath2Project.remove(trimTrailingPathSeparator(oldProject.getPath()));
			...
			lookupProjectPath2Project.put(trimTrailingPathSeparator(project.getPath()), project);
		...
		for (ProjectConfigSnapshot removedProject : removedProjects) {
			...
			lookupProjectPath2Project.remove(trimTrailingPathSeparator(removedProject.getPath()));
