...
/**
 * A factory for creating {@link ICompletionProposal}s that insert
 * new name-value-pairs into a JSON Document.
 * 
 * Example:
 * <code>
 * {
...
public class NameValuePairProposalFactory {
	@Inject private XtextTemplateContextTypeRegistry contextTypeRegistry;
	 ...
	 * @param trailingComma Specifies whether to include a trailing comma into the proposal text.
	 */
	public ICompletionProposal createNameValuePairProposal(ContentAssistContext context, boolean trailingComma) {
		...
		final TemplateContext tContext = new DocumentTemplateContext(getTemplateContextType(), context.getDocument(), context.getOffset(), 0);
		// pre-populate ${value} with "value" to avoid an invalid state in terms of syntax
	 ...
	 *
	 * @param context The content assist context to create the template for.
	 * @param trailingComma Adds a trailing comma to the name-value-pair.
	 */
