...
public abstract class AbstractSubGenerator implements ISubGenerator {
  ...
  public boolean shouldBeCompiled(final Resource input, final CancelIndicator monitor) {
    ...
    final String fileExtension = this._xpectAwareFileExtensionCalculator.getXpectAwareFileExtension(input.getURI());
    return (((((autobuildEnabled && ((this.hasValidFileExtension(input) || "n4jsx".equals(fileExtension)) || "jsx".equals(fileExtension))) && this.projectUtils.isSource(input.getURI())) && 
      ((this.projectUtils.isNoValidate(input.getURI()) || this.projectUtils.isExternal(input.getURI())) || ((EMFPlugin.IS_ECLIPSE_RUNNING && (!isXPECTMode)) || this.hasNoErrors(input, monitor)))) && (!this.isStaticPolyfillingModule(input))) && this.hasNoPolyfillErrors(input, monitor));
  }
  ...
  private boolean hasNoPolyfillErrors(final Resource input, final CancelIndicator monitor) {
    final N4JSResource resSPoly = this.projectUtils.getStaticPolyfillResource(input);
    if ((resSPoly == null)) {
      return true;
    }
  ...
  public void updateOutputPath(final IFileSystemAccess fsa, final String compilerID, final Resource input) {
    final String outputPath = this.projectUtils.getOutputPath(input.getURI());
  ...
  public Path calculateNavigationFromOutputToSourcePath(final IFileSystemAccess fsa, final String compilerID, final N4JSResource input) {
    final Optional<? extends IN4JSProject> projectctContainer = this.core.findProject(input.getURI());
  ...
  public boolean isNotStaticallyPolyfilled(final Resource resource) {
    if ((resource instanceof N4JSResource)) {
      ...
      if (_isStaticPolyfillAware) {
        boolean _hasStaticPolyfill = this.projectUtils.hasStaticPolyfill(resource);
