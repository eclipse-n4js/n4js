...
/**
 * Replaces {@link MigrationDeclaration} by a property access on the involved principle types.
 * (first input and first output parameters).
 *
...
public class MigrationTransformation extends Transformation {
	...
	public void transformMigration(MigrationDeclaration migrationDeclaration) {
		...
		if (migrationFunctionExpr.getFpars().isEmpty() || migrationDeclaration.getFrets().isEmpty()) {
			throw new IllegalStateException("A migration must have at least one input and at least one output parameter");
		}
		...
		if (!inputClassifier.getName().equals(outputClassifier.getName())) {
			throw new IllegalStateException("Principle in- and output parameter types of a migration must be the same.");
		}
	...
	private TClassifier getClassfierOfFormalParameter(FormalParameter formalParameter) {
		...
		if (!(entry instanceof SymbolTableEntryOriginal)) {
			throw new IllegalStateException("Types of migration parameters may not refer to IM-only or internal symbol table entries.");
		}
	...
	public ExpressionStatement migrationRegistrationStatement(FunctionExpression migrationFunction, String name, boolean isUpgrade) {
	...
	public FunctionExpression createMigrationFunctionDeclaration(MigrationDeclaration migrationDeclaration) {
		...
		ObjectLiteral returnParameterExpressions = TranspilerBuilderBlocks._ObjLit(Stream.of(returnParameterDeclarations)
	...
	public String generateMigrationName(MigrationDeclaration migrationDeclaration) {
