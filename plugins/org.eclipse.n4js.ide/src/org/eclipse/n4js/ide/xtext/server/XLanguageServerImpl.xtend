 ...
 * @since 2.11
 */
@Log class XLanguageServerImpl implements LanguageServer, WorkspaceService, TextDocumentService, LanguageClientAware, Endpoint, JsonRpcMethodProvider, IBuildListener {
	...
	@Inject
	def void setWorkspaceManager(XWorkspaceManager manager) {
	...
	XWorkspaceResourceAccess resourceAccess
	...
	override rename(RenameParams renameParams) {
		return requestManager.runRead[ cancelIndicator |
			...
			val resourceServiceProvider = uri.resourceServiceProvider
			val renameServiceOld = resourceServiceProvider?.get(XIRenameService)
	 ...
	 * @since 2.16
	 */
	protected def XWorkspaceManager getWorkspaceManager() {
		workspaceManager
	}
