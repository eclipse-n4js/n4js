 ...
 * @since 2.11
 */
@Log class XWorkspaceManager {

	@Inject Provider<XProjectManager> projectManagerProvider
	@Inject XIWorkspaceConfigFactory workspaceConfigFactory
	@Inject XIProjectDescriptionFactory projectDescriptionFactory

	XBuildManager buildManager

	Map<String, XProjectManager> projectName2ProjectManager = newHashMap
	...
	@Inject
	def void setBuildManager(XBuildManager buildManager) {
	...
	def XBuildable didChangeFiles(List<URI> dirtyFiles, List<URI> deletedFiles) {
	...
	def XProjectManager getProjectManager(URI uri) {
		val projectConfig = workspaceConfig.findProjectContaining(uri)
		return projectName2ProjectManager.get(projectConfig?.name)
	}

	def XProjectManager getProjectManager(String projectName) {
		projectName2ProjectManager.get(projectName)
	}

	def List<XProjectManager> getProjectManagers() {
	 ...
	 */
	@Deprecated
	def XBuildable didChange(URI uri, Integer version, Iterable<TextEdit> changes) {
	 ...
	 * @since 2.18
	 */
	def XBuildable didChangeTextDocumentContent(URI uri, Integer version, Iterable<TextDocumentContentChangeEvent> changes) {
		if (!openDocuments.containsKey(uri)) {
			LOG.error("The document " + uri + " has not been opened.")
			return [];
		}
	...
	def XBuildable didOpen(URI uri, Integer version, String contents) {
	...
	def XBuildable didClose(URI uri) {
