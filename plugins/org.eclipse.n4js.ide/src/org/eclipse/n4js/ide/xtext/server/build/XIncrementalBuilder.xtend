 ...
 * @since 2.9 
 */
class XIncrementalBuilder {
	
	@Data static class XResult {
		XIndexState indexState
		List<IResourceDescription.Delta> affectedResources
	}
	
	static class XInternalStatefulIncrementalBuilder {
	
		@Accessors(#[PROTECTED_SETTER, PROTECTED_GETTER]) extension XBuildContext context
		@Accessors(#[PROTECTED_SETTER, PROTECTED_GETTER]) XBuildRequest request
	
		@Inject XIndexer indexer
		...
		def XResult launch() {
			...
			return new XResult(request.state, resolvedDeltas)
		}
		...
		protected def void generate(Resource resource, XBuildRequest request, XSource2GeneratedMapping newMappings) {
			val serviceProvider = resource.resourceServiceProvider
		...
		@Singleton
		static class XURIBasedFileSystemAccessFactory {
			...
			def URIBasedFileSystemAccess newFileSystemAccess(Resource resource, XBuildRequest request) {
		...
		protected def createFileSystemAccess(IResourceServiceProvider serviceProvider, Resource resource) {
			return serviceProvider.get(XURIBasedFileSystemAccessFactory).newFileSystemAccess(resource, request)
		}
	
	}

	@Inject Provider<XIncrementalBuilder.XInternalStatefulIncrementalBuilder> provider
	@Inject extension OperationCanceledManager

	def XResult build(XBuildRequest request, (URI)=>IResourceServiceProvider languages) {
		build(request, languages, new DisabledClusteringPolicy())
	}
	
	def XResult build(XBuildRequest request, (URI)=>IResourceServiceProvider languages, IResourceClusteringPolicy clusteringPolicy) {
		val resourceSet = request.resourceSet
		val oldState = new XIndexState(request.state.resourceDescriptions.copy, request.state.fileMappings.copy)
		val context = new XBuildContext(languages
									, resourceSet
									, oldState
									, clusteringPolicy,
