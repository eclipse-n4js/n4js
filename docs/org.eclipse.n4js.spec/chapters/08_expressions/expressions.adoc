...
* If the operator is not ’+’ the types of the operands must be subtypes of number, boolean or null; otherwise any type could be used:

[math]
++++
\beginalign
\infer{\typeEnv \entails \type{AdditiveExpression}\ e \expectType \type{Expression}: \union{number,boolean,null}}{e.op \neq AdditiveOperator.ADD} \\
\infer{\typeEnv \entails \type{AdditiveExpression}\ e \expectType \type{Expression}: \type{any}}{e.op =  AdditiveOperator.ADD}
\endalign
++++
--

In JavaScript it is possible to subtract two non-strings, leading to `NaN`. In N4JS this is an error -- with the same argument `TypeError` could be an excuse for not doing any static type checks.
...
++++
\beginalign
\inferRule{nb}{nb(T)}{T \subtype = \type{number} \lor\ T \subtype \type{boolean}} \\
\inferRule{un}{un(T)}{T = \type{undefined} \lor\ T = \type{null}} \\
\inferRule{mnb}{mnb(T)}{nb(T) \lor\  (\mu(T)=\type{Union}\ \land\  \exists\  t\in T.typeRefs: mnb(t) } \\
\inferRule{mun}{mun(T)}{un(T) \lor\  (\mu(T)=\type{Union}\ \land\  \exists\  t\in T.typeRefs: un(T)} \\

\inferRule{toNum}{toNum(expr)}
    {\tee e.lhs: L \spc \tee e.rhs: R \spc (nb(L) \land\  nb(R)) \lor\  (((un(L) \lor\  un(R)) \land\  (nb(L) \lor\  nb(R) ))))} \\
\inferRule{mayNum}{mayNum(expr)}
    {\tee e.lhs: L \spc \tee e.rhs: R \spc (mnb(L) \land\  mnb(R)) \lor\  (((mun(L) \lor\  mun(R)) \land\  (mnb(L) \lor\  mnb(R) ))))}
...
Note: Adding two integers (int) leads to a number, since the result may not be represented as an (JavaScript) int anymore.
