...
* Numeric literals with a fraction, e.g. `2.0`, are always inferred to `number`, even if they represent integers in the range of int32.
* Numeric literals using scientific notation, e.g. `2e0`, are inferred to `int` iff they represent integers in the range of int32 (difference to Java!).
* Other numeric literals are inferred to `int` iff they represent integers in the range of int32, i.e. from $-2^{31}$ to $2^{31}-1$.
...
Until user story IDE-1881 is complete, all built-in operations always return a `number` even if all operands are of type `int`
and the type system treats types `int` and `number` as equivalent, i.e. `int <: number` and `number <: int`.
...
Template literals as defined in Section 12.2.9 of the ECMAScript 6 specification cite:[ECMA15a] are
supported in N4JS, including tagged template literals.

.Tagged Template Literals
[req,id=GH-24,version=1]
--
1. The tag expression of a tagged template literal containing expression segments `\$\{expr1}` ... `\$\{exprN}`
   with types $T_1$ ... $T_N$, respectively, must be of type `{function(Array<string>, T1, ..., TN):any}`.
+
For example, in `tagFn`hello \${'wonderful'} world \${2*21} !!!`` the expression `tagFn` must be of type
`{function(Array<string>,string,number):any}`.
2. The inferred type of a tagged template literal with a tag expression of type `{function(Array<string>,T1,...,TN):R}` is $R$.
+
For example, the inferred type of `(():number=>42)``` is `number`.
--
