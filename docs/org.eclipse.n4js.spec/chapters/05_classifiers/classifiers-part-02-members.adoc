...
Moreover, optionality has an effect in case of ternary expression $L <: R$ in which the left-hand side is a ternary expression, e.g. `l = b? trueExpr : falseExpr` whose `trueExpr` or `falseExpr` possibly recursively contains an expression of the kind mentioned above. In this case, the optionality effect is the more restricted optionality of `trueExpr` and `falseExpr`.
