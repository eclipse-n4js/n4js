...
See Section <<sec:call_and_construct_signatures>>.
...
* in cases 1, 2, and 5a and 5b, *both* fields and accessors (getters and setters) are optional.
That means, an optional data field, getter, or setter of $R$ needs not be present in $L$.
* in cases 3, 4, 5c, and 5d, only *getters* are optional, setters are not optional.
That means, an optional getter of $R$ needs not be present in $L$ and an optional field of $R$ requires only a setter in $L$. Note that these cases are more restricted than the cases 1/2 and 5a/5b.

Moreover, optionality has an effect in case of ternary expression $L <: R$ in which the left-hand side is a ternary expression, e.g. `l = b? trueExpr : falseExpr` whose `trueExpr` or `falseExpr` possibly recursively contains an expression of the kind mentioned above.
In this case, the optionality effect is the more restricted optionality of `trueExpr` and `falseExpr`.
...
In other words, in object literals (cases 1/2 and 5a/5b) neither optional getters, optional setters,
nor optional data fields are required. However, in case of new expressions and instances of
final classes (cases 3, 4, 5c, 5d) only optional getters are not required in a subtype;
...
It follows from the above definitions in Requirements <<Req-IDE-240500>> that cases 5a/5b and 5c are not
transitive across a chain of several `const` variables, whereas case 5d is transitive. For example:
...
The following example demonstrates how optionality behaves in ternary expressions.
...
// False expression is a new expression, so only getters in I are optional.
...
The next example shows why cases 3 and 4 (i.e. new expressions and instances of final classes) have to be handled in
a more restrictive manner than case 1 and 2 (i.e. object literals / function expressions).
...
== Call and Construct Signatures [[sec:call_and_construct_signatures]]

In `.n4jsd` files, interfaces can be provided with a call or construct signature. In this case, instances of such an interface can be invoked using
a call expression (in case of call signatures) and/or instantiated using a new expression (in case of construct signatures). Such call and construct
signatures in interfaces are inherited by extending interfaces. However, classes are unable to implement such interfaces, because it is not
possible for them to provide implementations for the call/construct signatures. The reason for this limitation is that call/construct signatures
are only intended to represent elements of plain Javascript libraries.

For example, given an `.n4jsd` file `ext.n4jsd` with

[source,n4jsd]
----
export external public interface ~IfcCall {
    (p: string): number; // call signature
}

export external public class Cls {}
export external public interface ~IfcConstruct {
    new(p: string): Cls; // construct signature
}
----

instances of these interfaces could be used in an `.n4js` file as follows:

[source,n4jsd]
----
import { IfcCall, IfcConstruct } from "ext"

let value1: IfcCall;
value1("hello"); // a value of type IfcCall can be invoked (returning a number)

let value2: IfcConstruct;
new value2("hello"); // a value of type IfcConstruct can be instantiated (creating an instance of Cls)
----


In addition to the above, classes in `.n4jsd` files can be provided with a call signature (not construct signature). Such a call signature
marks the constructor of this class as "callable", i.e. the constructor function may be invoked directly with a call expression.
Therefore, a call signature in a class is also referred to as a "callable constructor".

Note that call/construct signatures in interfaces and call signatures in classes are rather different and should be seen as two
separate features:

* call/construct signatures in interfaces are inherited by sub-interfaces, whereas call signatures in classes are not inherited
  by sub-classes.
* call/construct signatures in interfaces apply to values of the type, whereas call signatures in classes apply to the class
