...
`toLocaleString(locales: string|[string]=undefined, options: ~r~TEMP_NumberFormatOptions=undefined): string`::
...
is defined in https://www.ecma-international.org/ecma-402/4.0/index.html#numberformat-objects[ECMA-402 (Internationalization API Specification)].
...
definition is found in the runtime library `n4js-runtime-es2015`. 
