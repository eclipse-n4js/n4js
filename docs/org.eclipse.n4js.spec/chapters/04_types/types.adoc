...
Currently there are built-in types `Array2<T1,T2>`...`Array9<T1,...,T9>`.
...
N4JS supports boolean, numeric, and string literal types. These types are a specialization of the primitive types
`boolean`, `number`, and `string`, respectively, and have only a single valid value (not considering `undefined` and `null`).
Furthermore, enum literal types are supported, which are a specialization of the literal's containing enum type
and have the enum literal as their only valid value.

For example, using string literal types:
...
greeting = "hello"; // ok
greeting = "bye"; // ok
greeting = "see ya"; // error: "see ya" is not a subtype of union{"hello","bye"}.

let str: string;
str = greeting; // ok
greeting = str; // error: string is not a subtype of union{"hello","bye"}.
----

Similarly, with enum literal types:

[source,n4js]
----
enum Status { ON, OFF, UNKNOWN }
let onOff: Status.ON | Status.OFF;
onOff = Status.ON; // ok
onOff = Status.OFF; // ok
onOff = Status.UNKNOWN; // error: Status.UNKNOWN is not a subtype of union{Status.ON,Status.OFF}.

let status: Status;
status = onOff; // ok
onOff = status; // error: Status is not a subtype of union{Status.ON,Status.OFF}.

let onOffUnknown: Status.ON | Status.OFF | Status.UNKNOWN;
onOffUnknown = status; // ok, because the union comprises all literals of enum Status
----

Together with union types, these literal types can be used, for example, to realize anonymous enumerations:

[source,n4js]
----
function getStatus(): "on" | "off" | "unknown" {
    // ...
}
----

Literal types will be used as the implicitly inferred type of variables, fields, etc. only if they are immutable:

[source,n4js]
----
const immutable = "hello"; // type of 'immutable' will be "hello" (not string)
let mutable = "world";     // type of 'mutable' will be string (not "world")
----
...
=== ArrayN Short-Hand Syntax

The built-in ArrayN types (i.e. `Array2`, `Array3`, ... `Array9`, see <<ArrayN>>) are
...
[source,n4js]
----
let i3: Array3<string,number,Array<string>>;
----
