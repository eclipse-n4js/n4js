...
I.typeRefs \neq \emptyset
++++
2.  Only one nominal typed class must be contained in the intersection type:
+
[math]
++++
...
++++
4. If the intersection contains multiple references to the same generic type, a warning is produced if only undefined (or null) can be assigned to a type reference of this type. This is always the case except for the follwing situations: 
  ** all type argument corresponding to formal parameters without def-site variance are wildcards with an  upper bound (use "extends" or no bound) and all type arguments not defining a upper bound correspond to covariant (out) type parameters, and this constraint (IDE-27) holds for an intersection created from the upper bounds of the type arguments (or the lower bound of the type parameters).
  ** all type arguments are wildcards with lower bounds (since Object would be a solution) 
--
