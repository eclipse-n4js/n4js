...
main ::
  ...
  (see below).
yarnWorkspaceRoot ::
  Tells whether the component represents the root folder of a yarn workspace.
  This flag will be true iff the `package.json` contains yarn's top-level property
  "workspaces", no matter its value (i.e. this flag will be true even if the
  value of property "workspaces" is the empty array or an invalid value).
workspaces ::
  (array of strings) Value of top-level property "workspaces" in package.json,
  used by yarn to denote the projects contained in the yarn workspace.
...
apply to projects that represent the root folder of a yarn workspace; those projects to not have any source folder,
by default.
