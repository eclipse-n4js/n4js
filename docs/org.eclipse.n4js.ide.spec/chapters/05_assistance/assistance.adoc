...
* All import statements will be lexicographically sorted depending on the module specifier as major and the element name as minor key.
  However, the order of bare imports relative to each other as well as to other imports will never be changed.
  More precisely, given a bare import IB and another import I (which may be a bare import, too), the order of
  IB and I will not be changed by organize imports.
