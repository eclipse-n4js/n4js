...
startAndKeepVerdaccio:: enfore starting and suppress stopping of the test verdaccio (see <<sec:test-verdaccio>>)
...
freshly-compiled n4js-libs (the libraries located under top-level folder `/n4js-libs`) and is stopped at the
