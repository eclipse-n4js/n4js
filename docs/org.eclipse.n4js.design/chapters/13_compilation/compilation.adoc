...
The sources of the command line interface are located in in the `tools/` subfolder of the main git repository. They comprise of the package `org.eclipse.n4js.hlc` and corresponding test package `org.eclipse.n4js.hlc.tests`. (c.f. <<sec:tools,Tools>>).
...
Parameters for the headless-compiler-build are defined in the parent-pom located at `releng/org.eclipse.n4js.parent/pom.xml` with properties prefixed by `hlc.`.
