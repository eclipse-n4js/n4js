...
class ES_12_12_LabelledStatementEsprimaTest extends AbstractParserTest {
	...
	@Test
	def void testFor() {
		val program = 'start: for (;;) break start'.parseESSuccessfully
	...
	@Test
	def void testWhile() {
		val program = 'start: while (true) break start'.parseESSuccessfully
		val statement = program.scriptElements.head as LabelledStatement
	...
	def void testExpression() {
		val program = '__proto__: test'.parseESSuccessfully
		val statement = program.scriptElements.head as LabelledStatement
