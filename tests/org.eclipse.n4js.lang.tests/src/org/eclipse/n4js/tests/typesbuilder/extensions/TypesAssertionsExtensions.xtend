...
class TypesAssertionsExtensions {
	...
	def void assertExpectedTypesCount(String phase, Resource testResource, int expectedTypesCount) {
		...
		assertEquals(phase + ": Should have the expected count of types", expectedTypesCount, exportedScript.typesAndFunctions.size + exportedScript.exportedVariables.size)
	}
}
