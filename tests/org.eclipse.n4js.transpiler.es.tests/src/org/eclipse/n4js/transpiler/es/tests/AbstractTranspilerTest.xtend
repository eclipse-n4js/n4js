...
abstract class AbstractTranspilerTest {
	...
	def String cjsPatched(CharSequence raw) {
		return cjsPatched(raw, true);
	}
	def String cjsPatched(CharSequence raw, boolean patchWithCopyrightPreamble) {
		return '''
			«if(patchWithCopyrightPreamble) {
