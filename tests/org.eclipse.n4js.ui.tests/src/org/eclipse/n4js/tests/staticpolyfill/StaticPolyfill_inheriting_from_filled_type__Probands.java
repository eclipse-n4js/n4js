...
public class StaticPolyfill_inheriting_from_filled_type__Probands {
	...
	public static String validContent_Filled = """
			@@StaticPolyfillAware
	...
	// ********************************************************************************
	/** PART of IDEBUG-656 */
	public static String specStyle_Filled = """
			@@StaticPolyfillAware
	...
	/** PART of IDEBUG-656 */
	public static String specStyle_Filling = """
	...
	public static Pattern pattern_681_ENUM_MAP = Pattern.compile("Poly\\.ENUM_MAP");

	/** search pattern can be found in compiled code exactly once. */
	public static Pattern pattern_681_compiled_STRING_init = Pattern.compile("Poly\\.STRING\\s*=\\s*\"5\";");

	/**
	 * Platform independent regular expression for line breaks. Considers Windows and UNIX specific line breaks due to
	 * the platform dependent Xtend template building.
	 */
	private static String LINE_BREAK_REGEXP = "(\\r\\n|\\r|\\n)";

	/** search pattern can be found in compiled code exactly once. */
	public static Pattern pattern_681_compiled_ENUM_Init = Pattern
			.compile("Poly\\.ENUM_MAP\\s*=\\s*new\\s*DataMap\\(\\[" + LINE_BREAK_REGEXP + "\\s*\\[" + LINE_BREAK_REGEXP
					+ "\\s*'a'");
	...
	static void main(String[] args) {
		...
		Assert.assertTrue(StaticPolyfill_inheriting_from_filled_type__Probands.pattern_681_compiled_ENUM_Init
				.matcher(testCode).find());
		Assert.assertTrue(StaticPolyfill_inheriting_from_filled_type__Probands.pattern_681_compiled_STRING_init
				.matcher(testCode).find());
