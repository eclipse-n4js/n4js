...
class SingleProjectPluginUITest extends AbstractStaticPolyfillBuilderTest {
	...
	def void testStaticPolyfill_filling_updates_filled_module() throws Exception {
		...
		val actualContent2 = Files.asCharSource(compiledFile.location.toFile, Charset.defaultCharset).read();
		...
		val actualContent3 = Files.asCharSource(compiledFile.location.toFile, Charset.defaultCharset).read();
