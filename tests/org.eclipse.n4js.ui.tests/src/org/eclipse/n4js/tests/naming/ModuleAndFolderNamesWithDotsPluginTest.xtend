...
class ModuleAndFolderNamesWithDotsPluginTest extends AbstractBuilderParticipantTest {
	...
	@Inject private LibraryManager libraryManager;
	...
	def private void assertCorrectOutput(IFile module, CharSequence expectedOutput) {
		val moduleURI = createPlatformResourceURI(module.fullPath.toString, true);
		val result = projectTestsHelper.runWithNodeRunnerUI(moduleURI);
		val actualOutput = result.stdOut.trim;
		val expectedOutputTrimmed = expectedOutput.toString.trim;
		assertEquals("incorrect output when running " + module.name, expectedOutputTrimmed, actualOutput);
	}
}
