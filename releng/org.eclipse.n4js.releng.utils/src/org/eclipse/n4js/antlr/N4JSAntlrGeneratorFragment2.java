...
/**
 * Customization of the {@link XtextAntlrGeneratorFragment2} applying some massaging of the Parser implementation
 * generated by ANTLR.
 */
public class N4JSAntlrGeneratorFragment2 extends N4AntlrGeneratorFragment2 {
	private final static Logger LOGGER = Logger.getLogger(N4JSAntlrGeneratorFragment2.class);
	...
	protected void generateProductionGrammar() {
		...
		String absoluteParserFileName = getProjectConfig().getRuntime().getSrcGen().getPath() + "/"
				+ naming.getParserGrammar(getGrammar()).getGrammarFileName();

		massageGrammar(absoluteParserFileName, getCodeConfig().getEncoding());
	}
	...
	private void massageGrammar(String absoluteParserFileName, String encoding) {
		try {
			...
			String content = Files.asCharSource(new File(javaFile), Charset.forName(encoding)).read();
	...
	private String fixIdentifierAsKeywordWithEOLAwareness(String normalizedContent) {
		...
		String c2 = applyReplacement(c1, "ruleNoWhiteSpace.java.replacement");
		String c3 = applyReplacement(c2, "rulePrimaryExpression.java.replacement");
		return c3;
	}
